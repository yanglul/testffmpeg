...
pub fn write_audio<T: Sample>(data: &mut [T], samples: &mut Caching<Arc<SharedRb<Heap<T>>>, false, true>, _: &cpal::OutputCallbackInfo) {
    for d in data {
...
pub fn init_cpal() -> (cpal::Device, cpal::SupportedStreamConfig) {
...
pub fn play_mp4(file:&String) -> Result<(), ffmpeg::Error> {
    ...
    let audio_stream = match stream_config.sample_format() {
        SampleFormat::F32 => {
            build_output_stream_f32(&device,&stream_config.into(),consumer)
        },
...
pub fn build_output_stream_f32<T>(device:&cpal::Device,stream_config:&cpal::StreamConfig,mut consumer:Caching<Arc<SharedRb<Heap<T>>>, false, true>)-> Result<cpal::Stream, BuildStreamError>
where 
    T: cpal::SizedSample  + cpal::SizedSample + Send + 'static{
        device.build_output_stream(stream_config.into(), move |data , cbinfo| {
            // Copy to the audio buffer (if there aren't enough samples, write_audio will write silence)
